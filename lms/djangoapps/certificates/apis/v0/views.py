...
from lms.djangoapps.certificates.api import get_certificate_for_user, get_certificate_for_uuid
...
class CertificatesDetailViewByUUID(GenericAPIView):
    """
        **Use Case**

            * Get the details of a certificate for a specific uuid.

        **Example Request**

            GET /api/certificates/v0/certificates/{uuid}

        **GET Parameters**

            A GET request must include the following parameters.

            * uuid: A string representation of uuid for certificate.
    ...
    def get(self, request, uuid):
        ...
        Args:
            request (Request): Django request object.
            uuid (string): URI element specifying the certificate uuid.

        Return:
            A JSON serialized representation of the certificate.
        """

        user_cert = get_certificate_for_uuid(uuid=uuid)
        if user_cert is None:
            return Response(
                status=404,
                data={'error_code': 'no_certificate_for_uuid'}
            )
        return Response(
            {
                "username": user_cert.get('username'),
